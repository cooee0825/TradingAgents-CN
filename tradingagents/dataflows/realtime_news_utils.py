                        ...
                        for _, row in news_df.iterrows():
                            try:
                                ...
                                if time_str:
                                    ...
                                    logger.warning(
                                        f"[中文财经新闻] 新闻时间为空，使用当前时间"
                                    )
                                    publish_time = datetime.now()

                                # 检查时效性
                                if publish_time < datetime.now() - timedelta(
                                    hours=hours_back
                                ):
                                    skipped_count += 1
                                    continue
...
def get_realtime_stock_news(ticker: str, curr_date: str, hours_back: int = 6) -> str:
    ...
    if is_china_stock:
        ...
        try:
            ...
            logger.info(
                f"[新闻分析] 原始ticker: {ticker} -> 清理后ticker: {clean_ticker}"
            )

            logger.info(
                f"[新闻分析] 准备调用 get_stock_news_em({clean_ticker}, max_news=10)"
            )
            logger.info(f"[新闻分析] 开始从东方财富获取 {clean_ticker} 的新闻数据")
            start_time = datetime.now()
            logger.info(
                f"[新闻分析] 东方财富API调用开始时间: {start_time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}"
            )
            ...
            time_taken = (end_time - start_time).total_seconds()
            logger.info(
                f"[新闻分析] 东方财富API调用结束时间: {end_time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}"
            )
            ...
            if hasattr(news_df, "empty"):
                logger.info(
                    f"[新闻分析] 东方财富API返回DataFrame，是否为空: {news_df.empty}"
                )
                if not news_df.empty:
                    logger.info(
                        f"[新闻分析] 东方财富API返回DataFrame形状: {news_df.shape}"
                    )
                    logger.info(
            ...
            if not news_df.empty:
                ...
                logger.info(
                    f"[新闻分析] 成功获取 {news_count} 条东方财富新闻，耗时 {time_taken:.2f} 秒"
                )
    ...
    try:
        ...
        start_time = datetime.now()
        google_news = get_google_news.invoke(
            {"query": search_query, "curr_date": curr_date}
        )
        end_time = datetime.now()
        time_taken = (end_time - start_time).total_seconds()
