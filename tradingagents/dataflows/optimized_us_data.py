...
class OptimizedUSDataProvider:
    ...
    def get_stock_data(
        self, symbol: str, start_date: str, end_date: str, force_refresh: bool = True
    ) -> str:
        """
        获取美股数据 - 优先使用缓存
        ...
        if not force_refresh:
            ...
            if cache_key:
                ...
                logger.info(f"📊 [优化美股数据提供器] 缓存数据: {cached_data}")
                if cached_data:
        ...
        try:
            logger.info(
                f"🌐 从FINNHUB API获取数据--1: {symbol} start_date: {start_date} end_date: {end_date}"
            )
            self._wait_for_rate_limit()

            formatted_data = self._get_data_from_finnhub(symbol, start_date, end_date)
            logger.info(f"📊 [优化美股数据提供器] FINNHUB数据: {formatted_data}")
            ...
            try:
                ...
                if market_info["is_hk"]:
                    ...
                    try:
                        ...
                        # 备用方案：Yahoo Finance
                        logger.info(
                            f"🔄 使用Yahoo Finance备用方案获取港股数据: {symbol}"
                        )
