    ...
    try:
        if market_info["is_china"]:
            ...
            if "股票名称:" in stock_info:
                ...
                logger.debug(
                    f"📊 [基本面分析师] 从统一接口获取中国股票名称: {ticker} -> {company_name}"
                )
                return company_name
            else:
...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        logger.info(
            f"📊 [DEBUG] ===== 基本面分析师节点开始 ===== traceback: {traceback.extract_stack()}"
        )
        ...
        if toolkit.config["online_tools"]:
            ...
            # logger.info(f"📊 [DEBUG] 选择的工具: {tools}")
        ...
        logger.info(
            f"📊 [DEBUG] 工具名称: {tool_names} traceback: {traceback.extract_stack()}"
        )
        ...
        if GoogleToolCallHandler.is_google_model(fresh_llm):
            ...
            if tool_call_count > 0:
                ...
                try:
                    logger.debug(
                        f"📊 [DEBUG] 强制调用 get_stock_fundamentals_unified..."
                    )
                    # 安全地查找统一基本面分析工具
                    unified_tool = None
                    for tool in tools:
                    ...
                    if unified_tool:
                        logger.info(
                            f"🔍 [股票代码追踪] 强制调用统一工具，传入ticker: '{ticker}'"
                        )
