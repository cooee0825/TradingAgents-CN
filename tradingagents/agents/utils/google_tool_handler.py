...
class GoogleToolCallHandler:
    ...
    def handle_google_tool_calls(
        ...
        try:
                    ...
                    try:
                        if hasattr(msg, "content") and hasattr(msg, "__class__"):
                            ...
                            if msg_class_name in [
                                ...
                                # 转换为HumanMessage
                                logger.warning(
                                    f"[{analyst_name}] ⚠️ 转换非标准消息类型: {msg_class_name}"
                                )
            ...
            # 检查消息序列长度，对于Google模型需要更严格的控制
            total_length = sum(
                len(str(msg.content))
                for msg in safe_messages
                if hasattr(msg, "content")
            )
            logger.info(f"[{analyst_name}] 📊 消息序列总长度: {total_length} 字符")

            # Google模型对长输入比较敏感，降低限制
            if total_length > 50000:  # 对Google模型使用更保守的限制
                logger.warning(
                    f"[{analyst_name}] ⚠️ 消息序列过长 ({total_length} 字符)，进行优化..."
                )
                ...
                # 保留工具消息（对Google模型需要更积极的截断）
                for tool_msg in tool_messages:
                    if len(tool_msg.content) > 5000:  # 对Google模型使用更保守的截断
                        # 智能截断：保留开头和结尾的重要信息
                        content = tool_msg.content
                        if len(content) > 10000:
                            # 对于很长的内容，保留开头3000字符和结尾1500字符
                            truncated_content = (
                                content[:3000]
                                + "\n\n[注：中间数据已截断以确保Google模型处理效率]\n\n"
                                + content[-1500:]
                            )
                        else:
                            # 对于中等长度的内容，保留前5000字符
                        ...
                        logger.info(
                            f"[{analyst_name}] 🔧 工具消息截断: {len(tool_msg.content)} -> {len(truncated_content)} 字符"
                        )
                    else:
                ...
                optimized_messages.append(
                    HumanMessage(content=analysis_prompt_template)
                )
                ...
                logger.info(
                    f"[{analyst_name}] ✅ 消息序列优化完成，新长度: {new_total_length} 字符"
                )

                # 如果优化后还是太长，使用简化的分析提示
                if new_total_length > 40000:
                    logger.warning(
                        f"[{analyst_name}] ⚠️ 优化后仍然过长，使用简化分析提示"
                    )
                    # 替换最后的分析提示为简化版本
                    if safe_messages and isinstance(safe_messages[-1], HumanMessage):
                        simple_prompt = f"""基于工具获取的数据，请为{ticker}生成技术分析报告。
...
请生成技术分析报告："""
                        safe_messages[-1] = HumanMessage(content=simple_prompt)
                        logger.info(f"[{analyst_name}] 🔧 已切换到简化分析提示")
            else:
                logger.info(f"[{analyst_name}] ✅ 消息序列长度合理，无需优化")

            logger.info(
                f"[{analyst_name}] 📊 最终消息序列: {len(safe_messages)} 条消息"
            )
            ...
            try:
                ...
                logger.info(
                    f"[{analyst_name}] 📊 输入总长度: {total_input_length} 字符"
                )
                logger.info(f"[{analyst_name}] 📊 消息数量: {len(safe_messages)}")

                # 为Google模型设置生成参数，确保足够的输出长度
                if hasattr(llm, "model_kwargs"):
                    original_kwargs = (
                        llm.model_kwargs.copy() if llm.model_kwargs else {}
                    )
                    llm.model_kwargs = llm.model_kwargs or {}
                    llm.model_kwargs["max_output_tokens"] = 8192  # 设置最大输出长度
                    logger.info(f"[{analyst_name}] 🔧 设置max_output_tokens=8192")
                    final_result = llm.invoke(safe_messages)
                    llm.model_kwargs = original_kwargs  # 恢复原设置
                else:
                    logger.info(
                        f"[{analyst_name}] ⚠️ LLM没有model_kwargs属性，使用默认设置"
                    )
                    final_result = llm.invoke(safe_messages)
                ...
                # 添加更详细的结果检查
                logger.info(f"[{analyst_name}] 🔍 检查返回结果...")
                if hasattr(final_result, "content"):
                    content = final_result.content
                    logger.info(f"[{analyst_name}] 🔍 content类型: {type(content)}")
                    logger.info(
                        f"[{analyst_name}] 🔍 content长度: {len(content) if content else 0}"
                    )
                    if content:
                        logger.info(
                            f"[{analyst_name}] 🔍 content前100字符: {str(content)[:100]}"
                        )
                    else:
                        logger.warning(
                            f"[{analyst_name}] ⚠️ content为None或空字符串: {repr(content)}"
                        )
                else:
                    logger.error(f"[{analyst_name}] ❌ 返回结果无content属性")

                # 检查是否有其他有用的属性
                if hasattr(final_result, "response_metadata"):
                    logger.info(
                        f"[{analyst_name}] 📊 响应元数据: {final_result.response_metadata}"
                    )
                if hasattr(final_result, "usage_metadata"):
                    logger.info(
                        f"[{analyst_name}] 📊 使用统计: {final_result.usage_metadata}"
                    )
    ...
    def handle_simple_google_response(
        ...
        logger.info(
            f"[{analyst_name}] 📝 Google模型直接回复，长度: {len(result.content)} 字符"
        )
    ...
    def create_analysis_prompt(
        ...
        base_prompt = f"""现在请基于上述工具获取的真实数据，为{company_name}（{ticker}）生成一份详细深入的{analyst_type}报告。
...
4. **投资指导**：
   - 基于分析结果给出明确的投资建议（买入/持有/卖出）
   - 设定合理的目标价位和止损点
   - 详细说明投资风险和注意事项

5. **报告规格**：
   - 报告长度：不少于2000字，确保内容充实
   - 使用专业的中文术语
   - 确保分析的完整性和连贯性
...
**请注意：这是一份专业的投资分析报告，需要体现出高度的专业性和实用性。请充分展开每个分析要点，提供详尽的技术分析和投资指导。**

请现在开始生成完整的{analyst_type}报告："""
